...
import muster.codec.json.{Ac, Aliased}
import muster.output.OutputFormatter
import muster.util.SafeSimpleDateFormat
import org.specs2.mutable.Specification

class ObjectJsValueDecompositionSpec extends Specification {
  val format = PlayJsonCodec
  ...
  def write[T:Producer](value: T): JsValue = format.from(value)
  ...
  "A JsValue decomposer" should {
    ...
    "write a date" in {
      ...
      write(SafeSimpleDateFormat.Iso8601Formatter.parse(ds)) must_== JsString(ds)
    }

    "write a symbol" in {
      val sym = 'a_symbol_of_sorts
      val js = JsString(sym.name)
      write(sym) must_== js
    }
    ...
    "write a case class with a single list" in {
      val js: JsValue = parse(s"""{"lst":[1,2,3]}""")
    ...
    "write an object with a date" in {
      val date = new Date
      val ds = SafeSimpleDateFormat.Iso8601Formatter.format(date)
      val js: JsValue = parse(s"""{"date":"$ds"}""") //Json.toJson(Map("date" -> Json.toJson(ds)))
      write(WithDate(date)) must_== js
    }

    "write an object with a Symbol" in {
      val js: JsValue = parse(s"""{"symbol":"baz"}""")
    ...
    "write an option inside an option for a null" in {
      ...
      write(source) must_== JsObject("in" -> JsNull :: Nil)
    }
    ...
    "write with the custom implicit if one is provided" in {
      val js: JsValue = parse(s"""{"number":3854}""")
